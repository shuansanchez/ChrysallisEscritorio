...
namespace Chrysallis
{
    public partial class Control_de_Eventos : Form
    {
        ...
        {
            CreaModEventos nuevoEvento = new CreaModEventos(true, userL);
            nuevoEvento.ShowDialog();
        }

        private void buttonModificar_Click(object sender, EventArgs e)
        {
            CreaModEventos cambiaEvento = new CreaModEventos(false, (esdeveniments)dataGridViewEventos.SelectedRows[0].DataBoundItem, userL);
        ...
        private void Control_de_Eventos_Load(object sender, EventArgs e)
        {
            ...
            socioDelUsuario = ConsultaOrm.SelectSociosAdmins(userL);
            if (userL.id_rol == 5)
            {
                dataGridViewEventos.DataSource = ConsultaOrm.SelectEsdeveniments(socioDelUsuario);
            }
            else
            {
        ...
        private void Control_de_Eventos_Activated(object sender, EventArgs e)
        {
            ...
            else if (userL.id_rol == 6)
            {
                dataGridViewEventos.ReadOnly = true;
                dataGridViewEventos.DataSource = null;
                dataGridViewEventos.DataSource = ConsultaOrm.SelectEsdevenimentstodo(socioDelUsuario);
        ...
        private void toolStripButton3_Click(object sender, EventArgs e)
        {
            if (dataGridViewEventos.SelectedRows.Count > 0)
            {
                esdeveniments eventoBorrar = (esdeveniments)dataGridViewEventos.SelectedRows[0].DataBoundItem;
                ...
                if (dialogConfirmaBorra == DialogResult.OK)
                {
                    List<valoracions> listaValoraciones = ConsultaOrm.SelectValoracionesEvento(eventoBorrar);
                    if (listaValoraciones.Count <= 1)
                    {
                        //aqui borramos las valoraciones del evento a borrar
                        for (int i = 0; i < listaValoraciones.Count; i++)
                    ...
                    if (ConsultaOrm.SelectDocumentoEvento(eventoBorrar).Count > 0)
                    {
                        ConsultaOrm.DeleteDocumento(ConsultaOrm.SelectDocumentoEvento(eventoBorrar)[0]);
                    }
                    ConsultaOrm.DeleteEvento(eventoBorrar);
